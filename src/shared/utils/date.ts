/**
 * YYYYMMDD 형식의 문자열을 Unix timestamp (초 단위)로 변환한다.
 *
 * @param dateStr - YYYYMMDD 형식의 날짜 문자열 (예: "20240101")
 * @returns Unix timestamp (초 단위)
 * @throws {@link Error} 날짜 형식이 올바르지 않은 경우
 */
export const yyyymmddToUnixTimestamp = (dateStr: string): number => {
  ...
  // JavaScript Date의 month는 0부터 시작하므로 1을 뺀다.
...
/**
 * Unix timestamp (초 단위)를 YYYYMMDD 형식의 문자열로 변환한다.
 *
